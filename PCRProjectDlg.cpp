...
//		DisplayData("A device has been attached.");
		...
		if (!MyDeviceDetected || DeviceNameMatch(lParam))
		{
//			DisplayData("My device has been attached.");
			if (FindTheHID()) {
				m_TrimDlg.ResetTrim();
				m_GraDlg.DebugLog("Reset occurred");
			}
		}
...
//	ON_MESSAGE(WM_RegDlg_event,OnRegDlg)
	...
	ON_WM_SHOWWINDOW()
//	ON_WM_TIMER()
ON_BN_CLICKED(IDC_BUTTON_CHIPID, &CPCRProjectDlg::OnBnClickedButtonChipid)
END_MESSAGE_MAP()
...
*/
	...
	//===========Get Chip ID and temp==========

	CTheTestDlg  cidDlg;

	cidDlg.m_ChipID = g_ChipID;
	cidDlg.m_curTemp = g_curTemp;
	...
	SetDlgItemText(IDC_STATIC_CID, g_ChipID);
...
void CPCRProjectDlg::OnBnClickedButtonChipid()
{
	// TODO: Add your control notification handler code here
	CTheTestDlg  cidDlg;
