...
function [kk, bb, kk2, bb2, cc, hump, cerr, cerr2] = calibcol(high, low)
...
for j=2:limit
	...
	k = 0;
	startp = (j-1) * 16 + 1;
	endp = j * 16 - 3;		% exclude the points from the last (and first) few locations of the 16 period as most outlier data are there.
...
limit = limit - 1;					% exclude the last batch of data

for i=1:limit
	m(i) = 16 * i - 8;				% find mid-line
end
...
if cc > 6
  ...
  shrink = 0.0033 * cc;
									...
									% shrink is hardwired in auto_correct
...
plot(high, diff, 'o', high, offset);

line([0,256], [100,100], 'Linestyle', '--');					% visual bounds
line([0,256], [-100,-100], 'Linestyle', '--');
